...
- 即网络地址，主机在网络上的唯一标识。是网络层和以上各层使用的地址，是一种逻辑地址。
- 长度 32 位
...
每台主机都有一个`ARP高速缓存（ARP cache）`，里面有本局域网上各种主机和路由器的`IP地址`到`硬件地址`的映射，每条记录都有`生存时间`(如：10-20 分钟)
...
Question：既然链路层上传送的帧最终是按照`硬件地址`找到目标主机，为何要使用抽象的 IP 地址，不直接使用`硬件地址`进行通信？
...
- 第二条记录为本网络的直接交付
- 第三条记录为，到`192.168.6.0`网络的下一跳地址为`192.168.5.2`
- Flags 中的
  - `U`表示`路由是可用的`
...
| 分组转发时的优点 | 路由表是对网络拓扑变化的计算最优化 | 转发表的结构使查找过程最优化                     |
